...
def get_data(html_data: bytes) -> List[dict]:
    ...
    elements = combo_box.find_all(name='div', attrs='meal-section-mobile flexslider clear')
    phone_number = soup.find(name='p', attrs='delivery-phone').text.strip()

    combo_data = []
    for element in elements:
        if element.find(name='h5').text != 'Комбо-наборы':
            continue
        else:
            combo_elements = element.find_all(name='li')
            for combo in combo_elements:
                data = {}
                try:
                    # title
                    title_raw = combo.find(name='p', attrs='meal-name')
                    if not title_raw:
                        continue
                    title = combo.find(name='p', attrs='meal-name').text.strip()
                    data.update({"title": title})
                    # prices
                    price_raw = combo.find(name='span', attrs='meal-price')
                    if not price_raw:
                        continue
                    price = combo.find(name='span', attrs='meal-price').text.strip()
                    data.update({"new_price": price[:-3]})
                    
                    # img
                    img = combo.find(name='img').get('src').replace(' ', '%20')
                    ...
                    # weight
                    weight_raw = combo.find(name='div', attrs='meal-content-weight')
                    if weight_raw:
                        weight = combo.find(name='div', attrs='meal-content-weight').text.strip()
                        data.update({"weight": weight})
                    
                    # ingredients
                    ingredients = combo.find(name='div', attrs='meal-content-text').text.strip().replace('\xa0', ' ')
...
def main():
    try:
        ...
        ronny_combo_data = get_data(html_data)

        # utils.print_data(ronny_combo_data)
